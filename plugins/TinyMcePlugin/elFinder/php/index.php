...
session_set_cookie_params([
    'lifetime' => 0,          // Session expires when browser is closed
    'path' => '/',            // Available for all paths
    'domain' => '',           // Default domain
    'secure' => true,         // Only send over HTTPS
    'httponly' => true,       // Prevent JavaScript access
    'samesite' => 'None'      // Allow cross-site requests (important for iframes)
]);

// Start the session
if (session_status() === PHP_SESSION_NONE && !empty($_COOKIE['phpListSession'])) {
    session_id($_COOKIE['phpListSession']); // Restore previous session
}
session_start();
const ELFINDER_FILE_BASE_DIR = '../../../../../../images/elfinder/';

$user_directory = ELFINDER_FILE_BASE_DIR;
if (isset($_SESSION['logindetails'])) {
    $user_directory = ELFINDER_FILE_BASE_DIR. $_SESSION['logindetails']['id'] . '/';
}

if (!file_exists($user_directory)) {
    mkdir($user_directory, 0777, true);
    mkdir($user_directory .  '/.trash/', 0777, true);
    mkdir($user_directory .  '/.tmb/', 0777, true);
}
...
$opts = array(
    ...
    'roots' => array(
        ...
        array(
            ...
            'path'          => $user_directory,                 // path to files (REQUIRED)
            'URL'           => dirname($_SERVER['PHP_SELF']) . DIRECTORY_SEPARATOR . $user_directory, // URL to files (REQUIRED)
        ...
        array(
            'id'            => '1',
            'driver'        => 'Trash',
            'path'          => $user_directory . '/.trash/',
            'tmbURL'        => dirname($_SERVER['PHP_SELF']) . DIRECTORY_SEPARATOR . $user_directory . '.trash/.tmb/',
            'winHashFix'    => DIRECTORY_SEPARATOR !== '/', // to make hash same to Linux one on windows too
            'uploadDeny'    => array('all'),                // Recomend the same settings as the original volume that uses the trash
            'uploadAllow'   => array('image/x-ms-bmp', 'image/gif', 'image/jpeg', 'image/png', 'image/x-icon', 'text/plain', 'application/pdf'), // Same as above
